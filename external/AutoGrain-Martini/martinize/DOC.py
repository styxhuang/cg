...
forcefields = [str(ff).split('.')[-1] for ff in globals().values() if (isinstance(ff, type) and hasattr(ff,"ff"))]
...
#    ("-mixed",    Option(bool,                     0,    False, "Allow chains of mixed type (default: False)")),
    ...
    ("-ff",       Option(str,                      1,  "martini21", "Force field to use (default: martini21)")),
    ("-elastic",  Option(bool,                     0,    False, "Write elastic network")),
    ("-ef",       Option(float,                    1,      500, "Elastic network force constant (default: 500 kJ/mol/nm^2)")),
    ("-el",       Option(float,                    1,        0, "Elastic network lower cutoff (default: 0 nm)")),
    ("-eu",       Option(float,                    1,        0, "Elastic network upper cutoff (default: 0 nm)")),
    ("-ea",       Option(float,                    1,        0, "Elastic network decay factor (default: 0)")),
    ("-ep",       Option(float,                    1,        0, "Elastic network decay power (default: 0)")),
    ("-eb",       Option(str,                      1,     "all", "Elastic network beads (default: all)")),
    ("-em",       Option(float,                    1,        0, "Elastic network minimum force (default: 0)")),
    ("-multi",    Option(lists['multi'].append,    1,     None, "Multiscale chain (+)")),
    ]
...
def help():
    """Print help text and options."""
    print(options[0])
    print("Options:")
    for option, opt in options[1:]:
        print("%-10s %s"%(option, opt.description))
